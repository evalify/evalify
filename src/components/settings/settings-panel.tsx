...
export default function SettingsPage() {
    ...
    const handleThemeChange = async (newTheme: string) => {
        ...
        try {
            ...
            logger.error({ error: e, theme: newTheme }, "Failed to sync theme");
            error("Failed to save theme", { description: "Please try again later." });
            setTheme(previousTheme);
        }
    };

    const handleViewChange = async (newView: "list" | "grid") => {
        const previousView = view || "list";
        setView(newView);
        ...
        try {
            ...
            logger.error({ error: e, view: newView }, "Failed to sync view");
            error("Failed to save view", { description: "Please try again later." });
    ...
    const handleImageCropped = async (croppedImage: Blob) => {
        try {
            ...
            logger.error({ error: err }, "Error uploading image");
    ...
    const handleDeleteImage = async () => {
        ...
        try {
            ...
            logger.error({ error: err }, "Error deleting image");
    ...
    const handleSavePersonalInfo = async () => {
        ...
        if (personalEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(personalEmail)) {
            error("Invalid Email", { description: "Please enter a valid personal email address." });
            return;
        }
        ...
        if (dob) {
            ...
            if (isNaN(dateObj.getTime()) || dateObj > new Date()) {
                error("Invalid Date", { description: "Date of birth cannot be in the future." });
                return;
            }
        }

        if (gender && !["MALE", "FEMALE", "OTHER"].includes(gender)) {
            error("Invalid Gender", { description: "Please select a valid gender option." });
            return;
        }

        if (stateLocation && !INDIAN_STATES.includes(stateLocation)) {
            error("Invalid State", { description: "Please select a valid state from the list." });
            return;
        }

        if (city && city.trim().length > 255) {
            error("Invalid City", { description: "City name is too long. Max 255 characters." });
            return;
        }

        if (phoneNumber && !/^\d{10,15}$/.test(phoneNumber.replace(/[\s-]/g, ""))) {
            error("Invalid Phone", {
                description: "Please enter a valid phone number containing digits.",
            });
            return;
        }
        ...
        try {
            await updateProfileMutation.mutateAsync({
                ...
                personalEmail: personalEmail ? personalEmail.trim() : "",
                dob: dob,
                gender: gender,
                city: city ? city.trim() : "",
                state: stateLocation,
            ...
            logger.error({ error: err }, "Error updating personal information");
