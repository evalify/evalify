...
export function QuizMetadata({ data, updateData }: QuizMetadataProps) {
    ...
    const handleChange = (field: keyof QuizMetadataProps["data"], value: unknown) => {
        updateData({
            ...data,
            [field]: value,
        });
    ...
    // Helper: Get combined DateTime from date and time
    const getCombinedDateTime = (date: Date | undefined, time: string): Date | undefined => {
    ...
    const getDurationMinutes = (): number => {
        if (!data.duration?.value) return 60; // Default 60 minutes
        return data.duration.unit === "Hours" ? data.duration.value * 60 : data.duration.value;
    };

    // Helper: Set duration from hours and minutes
    const setDuration = (hours: number, minutes: number) => {
        const totalMinutes = Math.max(1, (hours || 0) * 60 + (minutes || 0));
        updateData({
            ...data,
            duration: { value: totalMinutes, unit: "Minutes" },
        });
    };
    ...
    useEffect(() => {
        ...
        const updates: Partial<QuizMetadataProps["data"]> = {};

        // Set default start time if missing
        if (!data.startDateTime.date || !data.startDateTime.time) {
            const now = new Date();
        ...
        // Set default duration if missing or zero
        if (!data.duration?.value || data.duration.value <= 0) {
            updates.duration = { value: 60, unit: "Minutes" };
            needsUpdate = true;
        }
    ...
    useEffect(() => {
        const startDT = getCombinedDateTime(data.startDateTime.date, data.startDateTime.time);
        const durationMins = getDurationMinutes();

        if (startDT && durationMins > 0) {
            const endDT = new Date(startDT.getTime() + durationMins * 60 * 1000);
            const currentEndDT = getCombinedDateTime(data.endDateTime.date, data.endDateTime.time);

            // Only update if different
            if (!currentEndDT || Math.abs(currentEndDT.getTime() - endDT.getTime()) > 1000) {
                updateData({
                    ...data,
                    endDateTime: {
                        date: endDT,
                        time: `${String(endDT.getHours()).padStart(2, "0")}:${String(endDT.getMinutes()).padStart(2, "0")}`,
                    },
                });
            }
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [data.startDateTime.date, data.startDateTime.time, data.duration.value, data.duration.unit]);
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="flex items-center gap-2">
                                            <Input
                                                ...
                                                min={0}
                                                max={23}
                                                value={Math.floor(getDurationMinutes() / 60)}
                                                onChange={(e) => {
                                                    const hours = Math.max(
                                                        0,
                                                        Number(e.target.value) || 0
                                                    );
                                                    const minutes = getDurationMinutes() % 60;
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between">
                                    <Tooltip>
                                        ...
                                        <TooltipContent>
                                            <p className="text-xs max-w-xs">
                                                Force the quiz to open in full screen mode to
                                ...
                                <div className="flex items-center justify-between">
                                    <Tooltip>
                                        ...
                                        <TooltipContent>
                                            <p className="text-xs max-w-xs">
                                                Students will only be able to start the quiz from
                                                the evalify application.
                                            </p>
                                        </TooltipContent>
                                    </Tooltip>
                                    <Switch
                                        ...
                                        onCheckedChange={(checked) =>
                                            handleSettingsChange("kioskMode", checked)
                                        }
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between">
                                    <Tooltip>
                                        ...
                                        <TooltipContent>
                                            <p className="text-xs max-w-xs">
                                                Randomly select questions from your question pool
                                                based on difficulty criteria
                                            </p>
                                        </TooltipContent>
                                    </Tooltip>
                                    <Switch
                                ...
                                <div className="flex items-center justify-between">
                                    <Tooltip>
                                        ...
                                        <TooltipContent>
                                            <p className="text-xs max-w-xs">
                                                Randomize the order of answer choices for each
                                                question
                                            </p>
                                        </TooltipContent>
                                    </Tooltip>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between">
                                    <Tooltip>
                                        ...
                                        <TooltipContent>
                                            <p className="text-xs max-w-xs">
                                                Allow participants to use a built-in calculator
                                                during the quiz
                                            </p>
                                        </TooltipContent>
                                    </Tooltip>
