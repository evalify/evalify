...
export default function CourseDetailsPage() {
  ...
  const fetchCourseQuizzes = async () => {
    try {
      ...
      toast.error(
        "Failed to load quizzes for this course. Please try again later.",
      );
    } finally {
      ...
      case "live":
        return (
          <Badge variant="default" className="bg-green-500">
            <AlertCircle className="mr-1 h-3 w-3" /> Live
          </Badge>
        );
      case "upcoming":
        return (
  ...
  const calculatePerformanceStats = () => {
    ...
    const quizScores = completedOrMissedQuizzes.map((quiz) => ({
      ...quiz,
      normalizedScore: quiz.status === "missed"
        ? 0
        : ((quiz.score || 0) / (quiz.totalScore || 100)) * 100,
    }));

    const totalScore = completedOrMissedQuizzes.reduce(
      (sum, quiz) =>
        sum + (quiz.status === "completed" ? (quiz.score || 0) : 0),
      0,
    );

    const totalPossibleScore = completedOrMissedQuizzes.reduce(
      (sum, quiz) => sum + (quiz.totalScore || 100),
      0,
    );

    // Calculate average percentage using normalized scores
    const averagePercentage =
      quizScores.reduce((sum, quiz) => sum + quiz.normalizedScore, 0) /
      quizScores.length;
    ...
    const scoreDistribution = {
      ...
      good:
        quizScores.filter((q) =>
          q.normalizedScore >= 70 && q.normalizedScore < 85
        ).length,
      average:
        quizScores.filter((q) =>
          q.normalizedScore >= 50 && q.normalizedScore < 70
        ).length,
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {course.staff.user?.name || course.staff.name}
                    </p>
                  </div>
                </div>
              )}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <span>Missed</span>
                <Badge variant="destructive">
                  {quizzes.filter((q) => q.status === "missed").length}
                </Badge>
              </div>
            </CardContent>
          </Card>
                      ...
                      ? (
                        <>
                          ...
                          <div className="pt-2">
                            ...
                            <p className="font-medium">
                              {performanceStats.totalScore.toFixed(1)} /{" "}
                              {performanceStats.totalPossibleScore.toFixed(1)}
                              {" "}
                              points
                            </p>
                          </div>
                        ...
                        <PieChart>
                          <Pie
                            data={[
                              ...
                              {
                                name: "Good (70-84%)",
                                value: performanceStats.scoreDistribution.good,
                              },
                              {
                                name: "Average (50-69%)",
                                value:
                                  performanceStats.scoreDistribution.average,
                              },
                              {
                                name: "Poor (<50%)",
                                value: performanceStats.scoreDistribution.poor,
                              },
                            ]}
                            cx="50%"
                            cy="50%"
                            ...
                            labelStyle={{
                              fill: isDarkMode
                                ? chartTheme.dark.pieLabelColor
                                : chartTheme.light.pieLabelColor,
                            }}
                          >
                          ...
                          <Tooltip
                            ...
                            contentStyle={{
                              backgroundColor: isDarkMode
                                ? chartTheme.dark.tooltipBg
                                : chartTheme.light.tooltipBg,
                              border: `1px solid ${
                                isDarkMode
                                  ? chartTheme.dark.tooltipBorder
                                  : chartTheme.light.tooltipBorder
                              }`,
                              borderRadius: "6px",
                              color: isDarkMode
                                ? chartTheme.dark.text
                                : chartTheme.light.text,
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          <CartesianGrid
                            strokeDasharray="3 3"
                            stroke={isDarkMode
                              ? chartTheme.dark.grid
                              : chartTheme.light.grid}
                          ...
                          <Tooltip
                            formatter={(
                              ...
                              name,
                              props,
                            ) => [`${value.toFixed(1)}%`, props.payload.title]}
                          ...
                          <Bar
                            ...
                            stroke="none"
                          >
                            {performanceStats.scoreByQuiz.map((
                              entry,
                              index,
                            ) => (
                              <Cell
                                key={`cell-${index}`}
                                fill={entry.status === "missed"
                                  ? "#FF0000"
                                  : (entry.score >= 85
                                    ? "#00C49F"
                                    : (entry.score >= 70
                    ...
                    <Card
                      ...
                      onClick={() => {
                        if (quiz.status === "live") {
                          ...
                          quiz.status === "completed" && quiz.showResult
                        ) {
                          ...
                          toast.info(
                            "Results are not available for this quiz yet.",
                          );
                        } else {
                          toast.info(
                            "This quiz is not available for taking yet.",
                          );
                        }
                      }}
                    >
