...
export default function StudentCoursesPage() {
    ...
    const { data, isLoading, error } = trpc.studentCourse.list.useQuery({
    ...
    const handleSearch = (search: string) => {
        ...
        setCurrentPage(1);
        track("student_courses_searched", { searchTerm: search });
    };

    const handleFilterType = (type: CourseType | "ALL") => {
        setFilterType(type);
        setCurrentPage(1);
        track("student_courses_filtered_by_type", { type });
    };

    const handleFilterStatus = (status: CourseStatus | "ALL") => {
        setFilterStatus(status);
        setCurrentPage(1);
        track("student_courses_filtered_by_status", { status });
    };

    const handlePageChange = (page: number) => {
        setCurrentPage(page);
        track("student_courses_page_changed", { page });
    };
    ...
    return (
        <div className="container py-6">
            <div className="mb-6">
                ...
                <p className="text-muted-foreground mt-2">View courses you are enrolled in</p>
            </div>
