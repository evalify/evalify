...
function ImagePreviewDialog(
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-4xl w-[95vw] h-[90vh] p-0 sm:p-0">
...
const getStorageKey = (quizId: string | string[], key: string) =>
    `quiz_${quizId}_${key}`;
...
const QuizPage = () => {
    ...
    useEffect(() => {
        ...
        const savedActiveFile = localStorage.getItem(
            `calculator_active_file_${quizId}`,
        );
            ...
            const res = await fetch("/api/quiz/save", {
                ...
                body: JSON.stringify({
                    ...
                    violations: violations.map((v) =>
                        `${new Date(v.timestamp).toISOString()}: ${v.message}`
                    ).join("\n"),
                }),
            });
    ...
    const handleDescriptiveAnswer = useCallback(
        (questionId: string, answer: string | string[]) => {
            setUserAnswers((prevAnswers) => {
                const currentAnswer = prevAnswers[questionId]?.[0];
                // Skip update if the answer hasn't changed
                if (Array.isArray(answer)) {
                    if (
                        JSON.stringify(prevAnswers[questionId]) ===
    ...
    const ViolationsCounter = useCallback(
        () => (
            <Popover>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        size="sm"
                        className={cn(
                            "fixed top-2 right-80 z-50",
                            violations.length > 0 &&
                                "bg-red-100 hover:bg-red-200 border-red-500 text-red-600",
                        )}
                    >
    ...
    useEffect(() => {
            ...
            try {
                ...
                if (!res.ok) {
                    switch (res.status) {
                        ...
                        default:
                            throw new Error(
                                data.error || "Failed to fetch quiz",
                            );
                    }
                }
                ...
                if (data.responses) {
                    setUserAnswers(JSON.parse(data.responses));
                }
        ...
        fetchQuiz();
        const fetchInterval = setInterval(fetchQuiz, 3 * 60 * 1000);
        return () => clearInterval(fetchInterval);
    }, [quizId, router]);

    useEffect(() => {
        if (!quizId || !userAnswers || Object.keys(userAnswers).length === 0) {
            return;
        }
        ...
        const isLongFormAnswer = () => {
            ...
            return type === "CODING" || type === "DESCRIPTIVE" ||
                type === "FILL_IN_BLANK";
        };

        const shouldUpdateImmediately = !isLongFormAnswer();
    ...
    }, [quizId, userAnswers, updateAnswers, currentQuestionIndex]);

    useEffect(() => {
        if (
            !questionParam || !quizId || !userAnswers ||
    ...
    }, [questionParam, updateAnswers, quizId, userAnswers]);
    ...
    const handleAnswerChange = (
        ...
        setUserAnswers((prevAnswers) => {
            ...
            const newAnswers = isChecked
                ...
                : {
                    ...prevAnswers,
                    [questionId]: currentAnswers.filter((id) =>
                        id !== optionId
                    ),
                };
            localStorage.setItem(
            ...
            <div className="space-y-4">
                ...
                {(() => {
                    switch (question.type) {
                        ...
                        case "CODING":
                            const codingLanguage = question.language ||
                                "python";
    ...
    const FileUpload = (
        { quizId, questionId, onUpload, currentFile }: FileUploadProps,
    ) => {
        const [isUploading, setIsUploading] = useState(false);
        const [dragActive, setDragActive] = useState(false);
        const [loadingState, setLoadingState] = useState<
            "uploading" | "removing" | "processing" | "completing" | "idle"
        >("idle");
    ...
    return (
        <div className="min-h-[90vh] flex flex-col bg-background">
                ...
                <div className="container mx-auto p-2 md:p-4">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
                        <div>
                            ...
                            <div className="flex items-center mt-1 md:mt-2 text-sm text-muted-foreground">
                                <Progress
                        ...
                        <div className="flex flex-row md:flex-row items-center justify-between md:justify-end w-full md:w-auto gap-2 md:gap-4">
                            ...
                            <div className="flex flex-col items-end gap-1">
                                <div
                                    className={cn(
                                        "flex items-center gap-2 px-3 py-1 md:py-2 rounded-full transition-colors duration-300",
                                        isTimeWarning
            ...
            <main className="flex-1 container mx-auto p-2 md:p-4 overflow-auto">
                <div className="flex flex-col lg:flex-row gap-4 lg:gap-8">
                    <div className="w-full lg:w-3/4 flex flex-col">
                        <Card className="flex-1 flex flex-col overflow-hidden">
                                    ...
                                    {isCalculatorEnabled && (
                                        <div className="flex items-center gap-2">
                                            <Dialog>
                                                ...
                                                <DialogContent className="w-[95vw] max-w-[1400px] h-[90vh] mx-auto">
                                                    <DialogTitle>
                                                        Calculator
                                                    </DialogTitle>
                                                    <CodeEditor
                            ...
                            <CardContent className="flex-1 overflow-y-auto mt-4 md:mt-6 pb-16 md:pb-6">
                                {renderQuestion(
                                    questions[currentQuestionIndex],
                                )}
                            </CardContent>
                        ...
                        <div className="sticky bottom-0 left-0 right-0 mt-2 md:mt-4 bg-background py-2 md:py-4 border-t">
                    ...
                    <div className="lg:hidden fixed bottom-0 left-0 right-0 border-t bg-background p-2 z-20">
                        <ScrollArea className="h-16">
                            <div className="flex gap-2 py-1">
                                {questions.map((q, index) => (
                                    <Button
                                        key={q.id}
                                        variant={currentQuestionIndex === index
                                            ? "default"
                                            : "outline"}
                                        className={cn(
            ...
            >
                <AlertDialogContent className="max-w-md mx-auto">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            You have answered {Object.keys(userAnswers).length}
                            {" "}
            ...
            {(isSubmitting || isAutoSubmitting) && (
                <div className="fixed inset-0 bg-background/80 flex items-center justify-center z-50">
                    <div className="flex flex-col items-center gap-2 p-4 max-w-[90%] text-center">
                        <LoaderCircle className="h-8 w-8 animate-spin text-primary" />
                        <p className="text-sm text-muted-foreground">
                            {isAutoSubmitting
                                ? `Automatically submitting your quiz... ${
                                    retryAttempts > 0
