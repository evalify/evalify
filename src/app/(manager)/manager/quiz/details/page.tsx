...
type Quiz = {
    ...
    courses: Course[];
    courseIds: string[];
    isEvaluated: "EVALUATED" | "UNEVALUATED";
};
        ...
        >
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex items-center gap-2">
                    <div
                        className={`flex items-center gap-2 px-3 py-1 rounded-full ${color} bg-opacity-10`}
                    >
                        {icon}
                ...
                <div className="space-y-3">
                    <div className="flex items-center text-gray-700 dark:text-gray-300">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span
                                className={`text-sm font-medium px-2 py-0.5 rounded-full inline-flex items-center ${
                                    quiz.isEvaluated === "EVALUATED"
            ...
            <div className="flex justify-between items-center mt-4">
                ...
                <div className="flex gap-2">
                    <Button
                        ...
                        size="sm"
                        onClick={() =>
                            router.push(`/manager/quiz/course/${quiz.id}`)}
                    ...
                    <Button
                        ...
                        onClick={() =>
                            router.push(
                                `/manager/quiz/course/result/${quiz.id}`,
                            )}
                        className="dark:bg-slate-700/50 dark:hover:bg-slate-600/50"
                    >
...
export default function QuizPage() {
    ...
    >("all");
    const [filterShowResult, setFilterShowResult] = useState<
        "all" | "show" | "hide"
    >("all");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    result.error ||
                        `Failed to ${editMode ? "update" : "create"} quiz`,
                );
            }
            ...
            toast.success(
                `Quiz ${editMode ? "updated" : "created"} successfully`,
            );
            setOpen(false);
            resetForm();
            fetchQuizzes();
            ...
            console.log(
                `Error ${editMode ? "updating" : "creating"} quiz:`,
                error,
            );
            toast.error(
                error instanceof Error
    ...
    const filteredAndSortedQuizzes = React.useMemo(() => {
        let filtered = quizzes.filter((quiz) => {
            const matchesSearch =
                ...
                quiz.description.toLowerCase().includes(
                    searchQuery.toLowerCase(),
                );

            const status = getQuizStatus(quiz.startTime, quiz.endTime).text
                .toLowerCase();
            const matchesFilter = filterStatus === "all" ||
                status === filterStatus;

            const matchesEvaluation = filterEvaluation === "all" ||
                (filterEvaluation === "evaluated" &&
                    quiz.isEvaluated === "EVALUATED") ||
                (filterEvaluation === "unevaluated" &&
                    quiz.isEvaluated === "UNEVALUATED");

            const matchesShowResult = filterShowResult === "all" ||
                (filterShowResult === "show" && quiz.settings.showResult) ||
                (filterShowResult === "hide" && !quiz.settings.showResult);

            return matchesSearch && matchesFilter && matchesEvaluation &&
                matchesShowResult;
        });
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        >
                            <TabsList className="bg-white dark:bg-gray-800">
                                <TabsTrigger
                                    value="table"
                                    className="data-[state=active]:bg-indigo-100 dark:data-[state=active]:bg-gray-700"
                                >
                            ...
                            <DialogContent className="sm:max-w-[800px] dark:border-gray-900 dark:bg-gray-950 max-h-[90vh] overflow-hidden flex flex-col">
                                <DialogHeader>
                                    ...
                                    <DialogDescription className="dark:text-gray-300">
                                        ...
                                        a quiz. Click save when you're done.
                                    </DialogDescription>
                                </DialogHeader>
                                <div className="flex-1 overflow-y-auto px-6 py-2">
                                    ...
                                    >
                                        ...
                                        <div className="space-y-2">
                                            <Label className="text-indigo-900 dark:text-indigo-100">
                                                Settings
                                            </Label>
                                            <div className="grid grid-cols-2 gap-4">
                                                {settingsFields.map((key) => (
                                                    <div
                                                        key={key}
                                                        className="flex items-center space-x-2 bg-white dark:bg-gray-900 p-2 rounded-lg shadow-sm"
                                                    >
                                                        <Checkbox
                                                            id={key}
                                                            checked={formData
                ...
                <div className="bg-white dark:bg-slate-800/50 rounded-lg shadow-md p-4 mb-6 backdrop-blur-sm">
                    <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Select
                                value={filterShowResult}
                                onValueChange={(value: any) =>
                                    setFilterShowResult(value)}
                            >
                                <SelectTrigger className="w-[140px]">
                                            ...
                                            filteredAndSortedQuizzes.map(
                                                (quiz) => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <TableCell
                                                                className={`${color} dark:text-gray-200`}
                                                            >
                                                                <div
                                                                    className={color}
                                                                >
            ...
            >
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            onClick={() =>
                                quizToDelete && handleDelete(quizToDelete)}
